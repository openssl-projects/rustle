...
pub enum Error {
    ...
    InvalidParameter,
    /// The operation is not valid in the context's current phase.
    InvalidState,
}
...
/// methods; optional methods have safe defaults and are registered only
...
pub trait Digest: Sized + 'static {
    ...
    /// Optionally write exactly `len` bytes, continuing an XOF's output stream.
    ///
    /// Check output capacity before advancing state. The implementation owns
    /// phase validation. The FFI adapter handles zero-length requests as
    /// successful no-ops without calling this method; see `EVP_DigestSqueeze(3)`.
    ///
    /// # Errors
    /// The default returns [`Error::Unsupported`]. Implementations may reject
    /// invalid state or insufficient storage. The adapter also rejects short
    /// writes; failure does not roll back output or implementation state.
    fn squeeze(&mut self, _out: &mut Output<'_>, _len: usize) -> Result {
    ...
    /// Optionally replace this context's state with a copy of `source`.
    ///
    /// OpenSSL's `digest_copyctx` returns void: implementations must complete
    /// the copy without recoverable failure and must also implement `dupctx`.
    /// The default is a no-op and does not register a callback. Omit this
    /// method if copying can fail; see `provider-digest(7)`.
    fn copyctx(&mut self, _source: &Self) {}
...
impl<D: Digest> DigestAlgorithm<D> {
    ...
    const ENTRIES: [OSSL_DISPATCH; 17] = {
        assert!(!D::HAS_COPYCTX || D::HAS_DUPCTX, "copyctx requires dupctx");
        assert!(
        ...
        let candidates = [
            ...
            if D::HAS_SQUEEZE {
                Some(OSSL_DISPATCH::digest_squeeze(Self::squeeze))
            } else {
                None
            },
            if D::HAS_DUPCTX {
            ...
            if D::HAS_COPYCTX {
                Some(OSSL_DISPATCH::digest_copyctx(Self::copyctx))
            } else {
                None
            },
        ...
        let mut entries = [OSSL_DISPATCH::END; 17];
        let mut source = 0;
        let mut dest = 0;
    ...
    unsafe extern "C" fn copyctx(outctx: *mut ffi::c_void, inctx: *mut ffi::c_void) {
        if outctx.is_null() || inctx.is_null() || outctx == inctx {
            return;
        }
        // SAFETY: the core lends a live D exclusively as the destination.
        let destination = unsafe { &mut *outctx.cast::<D>() };
        // SAFETY: the source is a live D from this table, disjoint from the
        // destination (self-copy was handled before creating references).
        let source = unsafe { &*inctx.cast::<D>() };
        destination.copyctx(source);
    }
    ...
    unsafe extern "C" fn squeeze(
        ...
        if !D::HAS_SQUEEZE
            || dctx.is_null()
            || isize::try_from(outlen).is_err()
            || (out.is_null() && outlen != 0)
        {
            return 0;
        }

        if outlen > 0 {
            // SAFETY: OpenSSL lends this live D exclusively during squeezing.
            let ctx = unsafe { &mut *dctx.cast::<D>() };
            // SAFETY: the core lends outlen writable bytes exclusively, disjoint
            // from ctx and outl; MaybeUninit permits uninitialized contents.
            let storage =
                unsafe { core::slice::from_raw_parts_mut(out.cast::<MaybeUninit<u8>>(), outlen) };

            let mut output = Output::new(storage);
            if ctx.squeeze(&mut output, outlen).is_err() || output.written() != outlen {
                return 0;
            }
        }
        // Like OpenSSL's shake_squeeze, permit the caller to omit outl.
        if !outl.is_null() {
            // SAFETY: when supplied, outl is a valid writable size slot;
            // a nonzero request committed exactly outlen bytes; zero is a no-op.
            unsafe { *outl = outlen };
        }
        1
    }
